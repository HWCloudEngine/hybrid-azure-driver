...
class AzureDriver(driver.ComputeDriver):
    ...
    def __init__(self, virtapi):
        ...
        try:
            ...
            msg = (_("Initialize Azure Adapter failed. reason: %")
                   % six.text_type(e))
            LOG.error(msg)
    ...
    def _precreate_network(self):
        ...
        try:
            ...
            if not subnet_info:
                ...
                subnet_info = {'address_prefix': '10.0.0.0/16'}
    ...
    def init_host(self, host):
        ...
        try:
            ...
            ex = exception.ResourceGroupCreateFailure(reason=six.text_type(e))
            raise ex
        ...
        try:
            ...
            ex = exception.StorageAccountCreateFailure(reason=six.text_type(e))
            raise ex
        ...
        try:
            ...
            ex = exception.StorageContainerCreateFailure(
    ...
    def get_info(self, instance):
        ...
        # azure may raise msrestazure.azure_exceptions CloudError
            ...
            if vm and hasattr(vm, 'instance_view') and \
                    hasattr(vm.instance_view, 'statuses'):
        ...
        for i in usages:
            if hasattr(i, 'name') and hasattr(i.name, 'value'):
                ...
                if usage_family == i.name.value:
                    cores_by_family = i.limit if hasattr(i, 'limit') else 0
    ...
    def _prepare_os_profile(self, instance, storage_profile, admin_password):
        ...
        if os_type == LINUX_OS:
            if instance.get('key_data'):
                key_data = six.text_type(instance.key_data)
        ...
        if image_properties and 'azure_type' in image_properties:
            if image_properties['azure_type'] == AZURE \
                ...
                ex = nova_ex.ImageUnacceptable(
                    image_id=image.id,
                    reson='Wrong parameters os Azure crated image!')
                raise ex
    ...
    def _copy_blob(self, container, blob_name, source_uri):
        try:
            ...
            ex = exception.BlobCopyFailure(reason=six.text_type(e),
                                           blob_name=blob_name,
    ...
    def _delete_blob(self, container, blob_name):
        try:
            ...
            message = (_("Volume blob: %s does not exist.") % blob_name)
            LOG.info(message)
            return True
        except Exception as e:
            ex = exception.BlobDeleteFailure(reason=six.text_type(e),
                                             blob_name=blob_name)
            raise ex
    ...
    def reboot(self, context, instance, network_info, reboot_type,
        ...
        try:
            ...
            ex = nova_ex.InstanceRebootFailure(reason=six.text_type(e))
            raise ex
    ...
    def power_off(self, instance, timeout=0, retry_interval=0):
        try:
            ...
            ex = nova_ex.InstancePowerOffFailure(reason=six.text_type(e))
            raise ex
    ...
    def power_on(self, context, instance, network_info,
        ...
        try:
            ...
            ex = nova_ex.InstancePowerOnFailure(reason=six.text_type(e))
            raise ex
    ...
    def rebuild(self, context, instance, image_meta, injected_files,
        ...
        try:
            ...
            ex = nova_ex.InstanceDeployFailure(reason=six.text_type(e))
            raise ex
    ...
    def migrate_disk_and_power_off(self, context, instance, dest,
        ...
        if not size_obj:
            raise exception.FlavorInvalid(flavor)
    ...
    def set_admin_password(self, instance, new_pass):
        # TODO(haifeng) doesn't support yet, can't work currently.

        if not self._check_password(new_pass):
            ex = exception.PasswordInvalid(instance=instance)
            raise ex
        vm = self._get_instance(instance.uuid)
        vm.os_profile.admin_password = new_pass
        self._create_update_instance(instance, vm)
        LOG.info(_LI("Reset Password of Instance in Azure finish"),
                 instance=instance)
    ...
    def _check_password(self, password):
        ...
        disallow = ["abc@123", "P@$$w0rd", "P@ssw0rd", "P@ssword123",
                    ...
                    "Password22", "iloveyou!"]
        return password not in disallow
    ...
    def detach_volume(self, connection_info, instance, mountpoint,
                      encryption=None):
        """Attach volume, remove volume info from vm parameters.
        """
    ...
    def _get_snapshot_blob_name_from_id(self, blob_id):
        return 'snapshot-{}.{}'.format(blob_id, VHD_EXT)
    ...
    def _cleanup_deleted_snapshots(self, context):
        ...
        for i in residual_ids:
            ...
            LOG.info(_LI('Delete residual snapshot: %s blob in Azure'), i)
    ...
    def _cleanup_deleted_nics(self):
        try:
            ...
            ex = exception.NetworkInterfaceListFailure(reason=six.text_type(e))
            raise ex
        residual_ids = [i.name for i in nics if not i.virtual_machine]
        to_delete_ids = set(self.residual_nics) & set(residual_ids)
        self.residual_nics = set(residual_ids) - set(to_delete_ids)
