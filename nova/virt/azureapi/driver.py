...
class AzureDriver(driver.ComputeDriver):
    ...
    def _get_new_size(self, instance, flavor):
        ...
        for i in sizes:
            if vm_size == i.name:
                ...
                return i.name
        LOG.error(_LE('Resize Instance, size invalid in Azure'),
                  instance=instance)
    ...
    def detach_volume(self, connection_info, instance, mountpoint,
        ...
        for i in range(len(data_disks)):
            if vhd_name == data_disks[i].name:
                ...
                break
        if not_found:
            LOG.error(_LE('Volume: %s was not attached to Instance!'),
                      vhd_name,
                      instance=instance)
            return
        self._create_update_instance(instance, vm)
    ...
    def _cleanup_deleted_snapshots(self, context):
        ...
        for i in zombied_ids:
            ...
            LOG.info(_LI('Delete zombie snapshot: %s blob in Azure'), i)
    ...
    def _cleanup_deleted_nics(self):
        ...
        for i in to_delete_ids:
            ...
            LOG.info(_LI('Delete zombie Nic: %s in Azure'), i)
    ...
    def _cleanup_deleted_os_disks(self):
        ...
        for i in blobs:
            if 'unlocked' == i.properties.lease.status \
                ...
                LOG.info(_LI("Delete zombie os disk: %s blob in"
                             " Azure"), i.name)
