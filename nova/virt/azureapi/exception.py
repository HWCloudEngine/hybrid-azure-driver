...
class OSTypeNotFound(exception.NotFound):
    msg_fmt = _("Unabled to decide OS type %(os_type) of instance.")
...
class BlobCopyFailure(exception.NovaException):
    msg_fmt = _("Unabled to copy blob %(blob_name) from %(source_blob)"
                " in Azure because %(reason)s")


class BlobDeleteFailure(exception.NovaException):
    msg_fmt = _("Unabled to delete blob %(blob_name)"
                " in Azure because %(reason)s")


class BlobNotFound(exception.NotFound):
    msg_fmt = _("blob %(blob_name) not found in Azure.")
